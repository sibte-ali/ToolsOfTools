 ...
 *
 * Devlys (also written Dev-Lys or DevLys) is a family of legacy ASCII-encoded
 * Hindi fonts widely used in Rajasthan, Madhya Pradesh, and Uttar Pradesh state
 * government offices, steno examinations, and newspaper compositing.
 *
 * Like Kruti Dev, Devlys stores ASCII characters (A-Z, a-z, symbols) that the
 * font software paints as Devanagari glyphs.  The key layout follows the
 * Remington QWERTY Hindi typewriter standard but with several key differences
 * from Kruti Dev 010, particularly in matra positions and some conjuncts.
 *
 * Unicode Devanagari uses the standard U+0900–U+097F block.
 */

// =============================================================================
// MAPPING TABLE — Devlys 010 ASCII → Unicode Devanagari
// Ordered longest-match first within groups for greedy-safe replaceAll.
// =============================================================================
...
export const DEVLYS_TO_UNICODE_MAP: [string, string][] = [
  ...
  // ── CONSONANTS ─────────────────────────────────────────────────────────────
  // Multi-char consonant representations (longer sequences first)
  ['[k', '\u0916'],  // ख
  ['?k', '\u0918'],  // घ
  ['.k', '\u0923'],  // ण
  ['Fk', '\u0925'],  // थ
  ...
  // Named conjunct shortcuts (Devlys uses same keys as Kruti Dev)
  ['K',  '\u091c\u094d\u091e'], // ज्ञ
  ['=',  '\u0924\u094d\u0930'], // त्र
  ['{',  '\u0915\u094d\u0937'], // क्ष
];
...
// =============================================================================
// DEVLYS → UNICODE DEVANAGARI
// =============================================================================

/**
 * Convert a Devlys 010-encoded string to Unicode Devanagari.
 *
 * Uses the same algorithm as krutiDevToUnicode:
 * 1. Pre-process reph 'Z' (typed AFTER the consonant in Devlys).
 * 2. Run map replacements.
 * 3. Reorder chhoti ee ki matra 'f' → ि.
...
// =============================================================================
// UNICODE DEVANAGARI → DEVLYS 010
// =============================================================================

/**
 * Convert Unicode Devanagari to Devlys 010 encoding.
 *
 * Algorithm (reverse of devlysToUnicode):
 * 1. Reorder chhoti ee ki matra ि → pre-base 'f'.
 * 2. Reorder reph (र्) → post-consonant 'Z'.
 * 3. Reverse-map with Unicode length descending (longest match first).
 */
